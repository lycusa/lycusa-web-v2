...
import { connectWallet, signMessage, isMetaMaskInstalled, onAccountsChanged, removeAccountsChangedListener } from '@/app/lib/wallet';
...
export default function WalletAuth({ onSuccess, onError }: WalletAuthProps) {
  ...
  const handleWalletAuth = async () => {
    ...
    try {
      ...
      const result = await verifyWalletSignature(siweMessage, signature, address);

      setTokens(result.accessToken, result.refreshToken);
      setMessage('Successfully authenticated!');
      onSuccess?.();
    } catch (err: any) {
      const errorMessage = err.response?.data?.error || err.message || 'Failed to authenticate with wallet';
      ...
      <div className="w-full space-y-6">
        <div className="text-center space-y-4">
          <div className="mx-auto w-20 h-20 bg-gradient-to-br from-orange-100 to-orange-50 rounded-2xl flex items-center justify-center">
            <svg className="w-12 h-12 text-orange-600" viewBox="0 0 40 40" fill="currentColor">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 max-w-sm mx-auto">
              To sign in with your wallet, you'll need to install the MetaMask browser extension first.
            </p>
          </div>
        </div>
        ...
        <div className="p-5 bg-gradient-to-br from-orange-50 to-amber-50 border border-orange-200 rounded-xl">
          <div className="flex items-start gap-3 mb-4">
            ...
            <div>
              <p className="text-sm font-medium text-gray-900 mb-1">Download MetaMask</p>
              <p className="text-xs text-gray-600">Free browser extension for Chrome, Firefox, and more</p>
            </div>
          </div>
          <div className="flex items-start gap-3 mb-4">
            <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center flex-shrink-0 shadow-sm">
            ...
            <div>
              <p className="text-sm font-medium text-gray-900 mb-1">Create or import wallet</p>
              <p className="text-xs text-gray-600">Set up your Ethereum wallet in minutes</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-gray-900 mb-1">Return and connect</p>
              <p className="text-xs text-gray-600">Come back to sign in with your wallet</p>
            </div>
          </div>
        </div>
    ...
    <div className="w-full space-y-6">
        ...
        <div className="p-4 bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-200 rounded-xl animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium text-purple-900 mb-0.5">Connected Wallet</p>
              <p className="text-sm font-mono text-purple-700 truncate">{walletAddress}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {!walletAddress && !loading && (
        <div className="p-4 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl border border-gray-200">
          <h4 className="text-sm font-semibold text-gray-900 mb-3">Why connect your wallet?</h4>
